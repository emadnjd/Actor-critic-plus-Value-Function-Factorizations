...
class Critic(nn.Module):
    def __init__(self, agent_num, state_dim, action_dim):
        ...
        input_dim = state_dim * agent_num + action_dim * agent_num
        self.fc1 = nn.Linear(input_dim, 64)
        self.fc2 = nn.Linear(64, 64)
        self.fc3 = nn.Linear(64, 1)  # Output a single Q-value

    def forward(self, state_action_input):
        x = F.relu(self.fc1(state_action_input))
        x = F.relu(self.fc2(x))
        return self.fc3(x)
...
class COMAAgent:
    def __init__(self, state_dim, action_dim, lr_c, lr_a, gamma, target_update_steps, buffer_capacity, batch_size, epsilon_start, epsilon_end, epsilon_decay):
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.gamma = gamma
        ...
        self.critic = Critic(agent_num=1, state_dim=state_dim, action_dim=action_dim)  # Critic for each agent
        self.critic_target = Critic(agent_num=1, state_dim=state_dim, action_dim=action_dim)
    ...
    def get_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            action = Categorical(dist).sample().item()  # .item() converts tensor to Python number
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        # Prepare the critic input
        input_critic = self.build_input_critic(states, actions)
        Q_values = self.critic(input_critic)
        Q_next = self.critic_target(self.build_input_critic(next_states, actions)).detach()
        ...
        advantages = Q_targets - Q_values.detach()
    ...
    def build_input_critic(self, states, actions):
        # Concatenate states and actions for the critic
        batch_size = states.size(0)
        actions_one_hot = torch.eye(self.action_dim)[actions.squeeze()].to(states.device)
        state_action_input = torch.cat([states, actions_one_hot], dim=-1)
        return state_action_input
...
def train(env, pred_agents, num_preys, total_timesteps, seed, max_episode_length=200):
    ...
    for t in range(1, total_timesteps + 1):
        ...
        if t % 100 == 0 or t == total_timesteps:  # Changed from 2000 to 100 for more frequent updates
...
def run_experiment(hyperparams, total_timesteps, num_seeds, epsilon_start, epsilon_end, epsilon_decay):
    ...
    for seed in range(num_seeds):
        pred_agents = [COMAAgent(state_dim, action_dim, lr_c, lr_a, gamma, target_update_steps, buffer_capacity, batch_size, epsilon_start, epsilon_end, epsilon_decay) for _ in range(num_predators)]
...
if __name__ == "__main__":
    ...
    # Save as .npy files
    np.save('pred_rewards_coma.npy', pred_rewards_array)
    np.save('episode_lengths_coma.npy', episode_lengths_array)
    np.save('cumulative_pred_rewards_coma.npy', cumulative_pred_rewards_array)  
    np.save('cumulative_episode_lengths_coma.npy', cumulative_episode_lengths_array)  

    plt.figure(figsize=(12, 10))  
    
    plt.subplot(2, 2, 1)  
    plt.plot(timesteps, pred_rewards)
    plt.title("Median Predator Reward_coma")
    plt.xlabel("Timesteps")
    plt.ylabel("Reward")

    plt.subplot(2, 2, 2)  
    plt.plot(timesteps, episode_lengths)
    plt.title("Average Episode Length_coma")
    plt.xlabel("Timesteps")
    plt.ylabel("Length")

    plt.subplot(2, 2, 3)
    plt.plot(timesteps, cumulative_pred_rewards)
    plt.title("Cumulative Median Predator Reward_coma")
    plt.xlabel("Timesteps")
    plt.ylabel("Cumulative Reward")

    plt.subplot(2, 2, 4)
    plt.plot(timesteps, cumulative_episode_lengths)
    plt.title("Cumulative Average Episode Length_coma")
    plt.xlabel("Timesteps")
